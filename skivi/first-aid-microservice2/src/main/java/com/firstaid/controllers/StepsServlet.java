        ...
        try {
            ...
            if (path.matches(STEP_OPTION)) {
                ...
                if (operation.equals("add")) {
                    ...
                    stepResource.setResourceId(resourceId);
                    stepResource.setStepId(stepId);
                    stepResourceRepository.remove(stepResource);
                }
            }  else {
        ...
        try {
            ...
            if (path.matches(RESOURCE_PATH)) {
                ...
                try {
                     ...
                     * catre un queue
                     */
                    notificationPublisher.publish("firstaid.step.update", "A new step has been added in firstAid: " + step.getStepHeader());
                    notificationPublisher.publish("firstaid.push.update", "A new step has been added in firstAid");
                } catch (DuplicateResourceException duplicateResourceException) {
                    response.sendError(HttpServletResponse.SC_CONFLICT);
                    return;
